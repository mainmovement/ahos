...
| Canonical security overlay (Lane B) | `architecture/security/gate.py` projects signals onto frozen CRITICAL keys and calls `discovery.security_gate.evaluate` (`security-overlay-v2`); PASS_WITH_UNKNOWN ⇒ INCOMPLETE; HIGH registry keys are not vetoes. TypeScript OPPORTUNITY alerts and `POST /api/paper` OPEN consume overlay PASS via `overlay_query.py` in addition to canonical BUY. |
...
| Lane-B evolution P5 (PR #93; **not AGI/ACI**) | Typed evidence → polarity → memory authorization → ObservationGrant → V4-PIN. Current law: `EVOLUTION_INDEX.md` preamble + `TEST_EVIDENCE.md` HEAD + `P5_FINAL_CLOSURE_GATE_REVIEW.md` (historical security-exit evidence). `GIT_MERGED = YES` (`74d0243ca7c07bb7a5c2738bfec84b9b3e4ea5b0`). `CODE_ON_MAIN = YES`. `GOVERNANCE_STATUS = CONTRADICTORY` (Git merge does not ratify governance). `PRODUCTION_AUTHORIZED = NO`. `RUNTIME_INTEGRATED = NO`. `RUNTIME_PROVEN = NO`. `SOAK_PROVEN = NO`. `SOAK_WIRED = NO`. `LIVE_TRADING_AUTHORIZED = NO`. `P5_SECURITY_EXIT = SECURITY-EXIT-CANDIDATE`; `P5_SECURITY_EXIT_DOES_NOT_AUTHORIZE_PRODUCTION = YES`. **Not** charter/world-model P5 (`ACI-GAP-002` OPEN). **Not** ops/n8n/Telegram P5. |
