...
| Canonical security overlay (Lane B) | `architecture/security/gate.py` projects signals onto frozen CRITICAL keys and calls `discovery.security_gate.evaluate` (`security-overlay-v2`); PASS_WITH_UNKNOWN ⇒ INCOMPLETE; HIGH registry keys are not vetoes. TypeScript OPPORTUNITY alerts and `POST /api/paper` OPEN consume overlay state via `overlay_query.py` (PASS only). |
