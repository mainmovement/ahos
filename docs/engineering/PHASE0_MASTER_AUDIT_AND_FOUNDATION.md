...
KNOWN_LIMITATIONS:
  ...
  - TypeScript second brain / GoPlus fail-open / Telegram HTML escape unchanged
    **SUPERSEDED 2026-09-09 (Telegram HTML):** `alerts.ts` +
    `telegram_ai/pump_alert.py` now escape `&` / `<` / `>` for `parse_mode=HTML`.
    See `docs/engineering/TELEGRAM_HTML_ESCAPE.md`.
    **SUPERSEDED 2026-09-09 (alerts web UI):** Command Center now fetches
    `/api/alerts`. Banner is hot only after live canonical BUY + overlay PASS
    re-check (`alert_banner.ts`). Not a buy signal. GoPlus missing-honeypot
