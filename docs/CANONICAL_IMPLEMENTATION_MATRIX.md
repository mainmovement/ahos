...
| Env key documentation | Config validation | Every key in `.env.example` | Documented AHOS_GATEWAY_URL + DATABASE_URL + alert keys | `.env.example` | `test_config_validation` | Fixed this pass | COMPLETE | — | — | — | — |
| Windows operator gate | Operator validation | Honest G1–G12 on Windows | Gate runner + handoff; Windows URI helper (Lane B) | `scripts/operator_validation_gate.py`, `config/paths.py`, `docs/WINDOWS_OPERATOR_HANDOFF.md` | `test_operator_validation_gate`, `test_paths_and_cross_platform` | Agent-host TEST_VERIFIED | PARTIAL | Windows artifact | High | Owner laptop | Run handoff; keep OPERATOR_READY NOT_VERIFIED until then |
| Lane-A Windows RO URI | Freeze governance | SQLite RO opens on Win paths | Still naive `file:{path}?mode=ro` in two Lane-A files | `discovery/observe_active.py`, `paper_trading/ledger.py` | freeze hash | Gap documented | BLOCKED_EXTERNAL | Freeze re-anchor | Med | Lane-A immutability | OA-LA owner decision |
