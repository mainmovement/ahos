...
type Provider = { provider: string; category: string; status: string; latencyMs: number | null; itemCount: number | null; messageFa: string | null };
type Snap = {
  generatedAt: string;
  state: {
    running: boolean;
  ...
  cycle: { id: number; status: string; durationMs: number | null; tokenCount: number | null; newsCount: number | null; opportunityCount: number | null; unknownShare: number | null; notesFa: string | null } | null;
  market: {
    regime: string;
  ...
  paper: Array<{ id: number; symbol: string; status: string; entryPrice: number | null; lastPrice: number | null; maxFavorable: number | null; maxAdverse: number | null }>;
  ...
  council: Array<{ tokenKey: string; verdict: string; summaryFa: string; watchCount: number; rejectCount: number; abstainCount: number }>;
...
export default function CommandCenter() {
  ...
  const watch = async (o: Opp) => {
    ...
    await fetch("/api/watch", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ tokenKey: o.tokenKey, symbol: o.symbol, chain: o.chain, address: o.address, thesisFa: "از داشبورد" }),
    });
    beep(true, sound);
    setBusy(null);
    await load();
  ...
  const paper = async (o: Opp) => {
    ...
    await fetch("/api/paper", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ tokenKey: o.tokenKey, symbol: o.symbol, chain: o.chain, address: o.address, entryPrice: price, thesisFa: "کاغذی از داشبورد" }),
    });
    beep(true, sound);
    setBusy(null);
    await load();
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      <div
        ...
        style={{ backgroundImage: "url(/images/ahos-hero.jpg)", backgroundSize: "cover", backgroundPosition: "center" }}
      />
      <div
        className="pointer-events-none absolute inset-0 opacity-40 grid-move"
        style={{
          backgroundImage: "url(/images/ahos-grid.jpg)",
          backgroundSize: "cover",
          mixBlendMode: "screen",
        }}
      />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-[#040714]/40 via-[#040714]/70 to-[#040714]" />
      ...
      <header className="relative z-10 mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-4 px-4 py-5 md:px-8">
        <div className="flex items-center gap-3">
          <img src="/images/ahos-mark.png" alt="AHOS" className="h-12 w-12 rounded-xl object-cover ring-1 ring-cyan-300/40" />
          <div>
        ...
        <section className="scene">
          <div className="glass relative overflow-hidden rounded-[28px] p-6 md:p-8">
            <div className="absolute -left-10 top-6 hidden h-44 w-44 float-y md:block">
              ...
              <img src="/images/ahos-orb.jpg" alt="" className="orb-spin h-full w-full rounded-full object-cover opacity-90" />
            </div>
            <div className="md:pr-40">
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <Hud
                  className="min-w-[160px] bg-gradient-to-l from-cyan-400/30 to-fuchsia-500/20 text-base"
                  loading={busy === "start"}
              ...
              <p className="mt-4 text-sm text-white/70">
                آخرین چرخه: {snap?.state.lastCycleStatus ?? "UNKNOWN"} — تعداد {faNum(snap?.state.cycleCount, 0)} — توکن‌ها {faNum(snap?.cycle?.tokenCount, 0)} — اخبار {faNum(snap?.cycle?.newsCount, 0)}
                ...
                <article key={o.id} className="glass rounded-[24px] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="m-0 text-xs text-white/50">{o.name} · {o.tokenKey}</p>
                    </div>
                ...
                {(snap?.paper || []).map((p) => (
                  <div key={p.id} className="mb-2 rounded-2xl bg-white/5 p-3 text-sm">
                    ...
                    <div className="text-white/60">
                      ورود {faUsd(p.entryPrice)} · الان {faUsd(p.lastPrice)} · MFE {p.maxFavorable ?? "نامشخص"} · MAE {p.maxAdverse ?? "نامشخص"}
                    </div>
                  </div>
                ))}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={() => setSelected(null)}>
          <div className="glass max-h-[85vh] w-full max-w-2xl overflow-auto rounded-[28px] p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="mt-0 text-2xl">
              {selected.symbol} — چرا این حکم؟
            </h3>
            <p>تصمیم {selected.decision} · اطمینان {selected.confidence} · شورا {selected.councilVerdict}</p>
            <h4>شواهد مثبت</h4>
            <ul>{(selected.reasonsFa || []).map((x) => <li key={x}>{x}</li>)}</ul>
            <h4>ریسک</h4>
            <ul>{(selected.risksFa || []).map((x) => <li key={x}>{x}</li>)}</ul>
            <h4>UNKNOWN</h4>
            <ul>{(selected.unknownsFa || []).map((x) => <li key={x}>{x}</li>)}</ul>
            <h4>داده کم</h4>
...
function Metric({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="rounded-2xl bg-black/30 p-3 ring-1 ring-cyan-300/15">
...
function StatusPill({ status }: { status: string }) {
  const tone =
    status === "SUCCESS" || status === "OK" || status === "WATCH" || status === "HIGH"
      ...
      : status === "REJECT" || status === "DOWN" || status === "HONEYPOT" || status === "DISABLED"
