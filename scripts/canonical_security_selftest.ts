...
describe("shouldAlertOpportunity — former bypass", () => {
  ...
  it("BEFORE-behavior: UNKNOWN/OBSERVED/empty used to alert; AFTER: denied", () => {
    assert.equal(
      shouldAlertOpportunity(opp({ securityStatus: "UNKNOWN", canonicalSecurityState: null }), st, gate),
      false,
    );
    assert.equal(
      shouldAlertOpportunity(opp({ securityStatus: "OBSERVED", canonicalSecurityState: undefined }), st, gate),
      false,
    );
    assert.equal(
      shouldAlertOpportunity(opp({ securityStatus: "", canonicalSecurityState: "" }), st, gate),
      false,
    );
  });

  it("canonical PASS + high opportunity → alert allowed", () => {
    assert.equal(
      shouldAlertOpportunity(opp({ canonicalSecurityState: "PASS" }), st, gate),
      true,
    );
  });
  ...
  it("canonical REJECT/INCOMPLETE/STALE/UNKNOWN + high opportunity → no alert", () => {
    for (const s of ["REJECT", "INCOMPLETE", "STALE", "UNKNOWN"]) {
      assert.equal(
        shouldAlertOpportunity(opp({ canonicalSecurityState: s, rankScore: 0.99 }), st, gate),
        false,
        s,
      );
    }
  ...
  it("local UNKNOWN + canonical PASS → PASS is authoritative (alert allowed)", () => {
    assert.equal(
      shouldAlertOpportunity(
